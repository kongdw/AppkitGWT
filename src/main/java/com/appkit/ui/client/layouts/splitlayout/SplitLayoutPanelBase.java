 ...
 * </p>
 */
public abstract class SplitLayoutPanelBase extends DockLayoutPanel implements HasResizeHandlers {

    private SplitLayoutAppearance appearance;
    static SplitLayoutAppearance DEFAULT_APPEARANCE = GWT.create(DefaultSplitLayoutAppearance.class);
        ...
        protected int getEventPosition(Event event) {
            ...
            if (TouchEvents.isTouchEvent(event)) {
                if (event.getTouches().length() > 0) {
                    return event.getTouches().get(0).getClientX();
                }
            }
    ...
    abstract class Splitter extends Widget {
        ...
        public Splitter(Widget target, boolean reverse) {
            ...
            sinkEvents(Event.ONMOUSEDOWN | Event.ONMOUSEUP | Event.ONMOUSEMOVE
                    | Event.ONDBLCLICK | Event.ONTOUCHSTART | Event.ONTOUCHMOVE | Event.ONTOUCHEND);
        }
        ...
        public void onBrowserEvent(Event event) {
           ...
           * which is the greater of the scroll size and the test.client size.
           */
                    ...
                    if (layout.direction == Direction.EAST ||
                            layout.direction == Direction.WEST) {
                        RootPanel.getBodyElement().getStyle().setProperty("cursor", "ew-resize");
                    } else {
                        RootPanel.getBodyElement().getStyle().setProperty("cursor", "ns-resize");
                    }
            ...
            if (layoutCommand == null) {
                layoutCommand = new ScheduledCommand() {
                    ...
                    public void execute() {
                        layoutCommand = null;
                        forceLayout();
                        ResizeEvent.fire(that, target.getOffsetWidth(), target.getOffsetHeight());
    ...
    class VSplitter extends Splitter {
        ...
        protected int getEventPosition(Event event) {
            ...
            if (TouchEvents.isTouchEvent(event)) {
                if (event.getTouches().length() > 0) {
                    return event.getTouches().get(0).getClientY();
                }
            }
    ...
    public SplitLayoutPanelBase(SplitLayoutAppearance appearance) {
        this(appearance, DEFAULT_SPLITTER_SIZE);

    }

    /**
     * Construct a new {@link SplitLayoutPanelBase} with the default splitter size of
     * 8px.
     */
    public SplitLayoutPanelBase() {
        this(DEFAULT_APPEARANCE, DEFAULT_SPLITTER_SIZE);
    }

    public SplitLayoutPanelBase(int splitterSize) {
        this(DEFAULT_APPEARANCE, splitterSize);

    }

    /**
     * Construct a new {@link SplitLayoutPanelBase} with the specified splitter size
     * in pixels.
     *
     * @param splitterSize the size of the splitter in pixels
     */
    public SplitLayoutPanelBase(SplitLayoutAppearance appearance, int splitterSize) {
        super(Unit.PX);

        this.appearance = appearance;
        this.splitterSize = splitterSize;
        setStyleName(appearance.css().gwtSplitLayoutPanel());
    ...
    public SplitLayoutAppearance getAppearance() {
        return appearance;
    }
